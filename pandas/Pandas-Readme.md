...
**Pandas** is a Python library for working with **tables** of data (like spreadsheets). You can load, filter, sort, aggregate, and clean data easily. This guide uses short explanations and lots of examples so you can revise quickly.
...
8. [Filtering Rows (boolean indexing)](#8-filtering-rows-boolean-indexing)
...
**Install Pandas (run in terminal):**
...
**What it does:** We use `pd` as the short name for pandas. Almost every example below starts with `import pandas as pd`.

---

## 2. Series – One Column of Data

A **Series** is a single column: a list of values with an index (row labels).
...
A **DataFrame** is a table: rows and columns. Each column is a Series.

**Think of it as:** Rows = records, Columns = attributes (like in Excel or SQL).

---

## 4. Creating DataFrames

**Syntax (from a dictionary):**
...
# First row is treated as column names by default
```
...
```python
df.to_csv('output.csv', index=False)  # don't write row index to file
```

---

**Read Excel (need openpyxl):**
...
**Example DataFrame for the rest of the guide:**
...
print(df.head(10)) # first 10 rows – good for a quick check
```

**What it does:** Shows the top N rows. Default is 5; pass a number to see more (e.g. 10).
...
**What it does:** Shows the bottom N rows. Default is 5. Useful to see the end of the data.
...
```python
df.info()
# Shows: columns, non-null count, dtype (int64, object, float64, etc.)
```

**What it does:** Helps you see if there are missing values and what type each column is.
...
**What it does:** Count, mean, std, min, quartiles, max for numeric columns only.
...
**What it does:** Returns the list of column names. Check this before renaming or selecting columns.
...
**What it does:** Shows each column’s data type (e.g. int64, float64, object for text). Use when you need to know types without running `info()`.
...
```python
df['Name']
df.Name   # same, but avoid if column name has spaces
```
...
**Syntax:** `df[condition]` – keep only rows where the condition is True.
...
**Example 2 – AND (both conditions):**

```python
df[(df['Dept'] == 'IT') & (df['Salary'] >= 55000)]
```

**What it does:** Use `&` for AND. Wrap each condition in parentheses.
...
```python
df[(df['Dept'] == 'HR') | (df['Age'] < 30)]
```

**What it does:** Use `|` for OR. Wrap each condition in parentheses.
...
**What it does:** `~` means NOT (invert the boolean Series).

---

**Example 5 – isin (match any value in a list):**
...
You can add a column by assigning a **list** of values. The list must have the **same length** as the number of rows in the DataFrame (one value per row).

```python
# Our df has 5 rows (Alice, Bob, Charlie, Diana, Eve), so we need 5 values
df['Location'] = ['NYC', 'LA', 'Chicago', 'Boston', 'Seattle']
```

**What it does:** Adds a new column **Location**. The 1st value goes to the 1st row, the 2nd to the 2nd row, and so on. Use this when you have **new information** (e.g. from another source) that you want to attach to each row. If the list is shorter or longer than the number of rows, you’ll get an error.
...
# Sort by Dept A–Z, then by Salary high to low within each Dept
```
...
```python
df.isna()           # True where value is NaN/None (alias: df.isnull())
df.isnull().sum()   # count of NaN per column (same as isna().sum())
df.dropna()         # drop rows with any NaN
df.dropna(subset=['Salary'])  # drop rows where Salary is NaN
```

**What it does:** `isna()` and `isnull()` are the same; both return True for missing values. `.sum()` gives the count of missing values per column.

**Fill missing values:**

```python
df.fillna(0)                                    # replace all NaN with 0
df['Salary'].fillna(df['Salary'].mean())        # fill one column with its mean
df.fillna(df.mean(numeric_only=True))           # fill all numeric columns with their column mean
```

**What it does:** `fillna()` replaces NaN. You can use a single value (e.g. 0), or the column mean. `df.mean(numeric_only=True)` computes the mean of only numeric columns, so you can fill the whole DataFrame without affecting text columns.
...
**What it does:** Converts a column of date strings (e.g. '2024-01-15') into proper **datetime** objects. After this you can sort by date, filter by date range, and use `.dt` accessor.
...
**What it does:** Once the column is datetime, use **`.dt.year`**, **`.dt.month`**, **`.dt.day`** to get numeric year, month, and day. Useful for grouping by year/month or filtering (e.g. only 2024).
...
```python
df = pd.DataFrame({'date': ['2024-01-15', '2024-02-20', '2023-12-01']})
df['date'] = pd.to_datetime(df['date'])
df['year'] = df['date'].dt.year
df['month'] = df['date'].dt.month
#   date        year  month
...
**What it does:** Returns an array of **distinct values** in that column (no duplicates). Use it to see what categories or levels exist (e.g. which departments, which cities).

---

**Count of each value (frequency):**
...
**What it does:** Counts how many times each value appears. Returns a Series: value → count. Like a frequency table. Use it to see the distribution of a categorical column.
